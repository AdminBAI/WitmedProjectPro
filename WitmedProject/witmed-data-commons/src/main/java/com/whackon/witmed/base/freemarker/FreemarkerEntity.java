...
public class FreemarkerEntity {
	public static void main(String[] args) {
		...
		try {
			...
			// 循环结果集，创建 Property类型结合
			List<Property> propertyList = new ArrayList<Property>();
			while (resultSet.next()){
				// 提取数据
				String type = resultSet.getNString("data_type");
				String name = resultSet.getNString("column_name");
				String comment = resultSet.getNString("column_comment");
				// 将数据库的数据类型变为Java 的数据类型
